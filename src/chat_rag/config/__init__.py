...
#:
#: It sits in the *repository* root, beside ``pyproject.toml`` -- three levels
#: above this package's ``config`` directory (``src/chat_rag/config``), not one
#: as it was when these packages were the repository root themselves. Which
#: file a library reads, and whether it should read one at all, is the question
#: L2 answers; this only keeps pointing at the same file the move found.
_REPOSITORY_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.dirname(os.path.abspath(__file__)))))
ENV_FILE = os.path.join(_REPOSITORY_ROOT, ".env")
