...
| `docker.build` | Docker is not running, or the image build genuinely broke |
| `docker.run` | the stack did not come up healthy from a clean clone: read the compose logs it prints. "the schema was not built from empty" means the migration step did not run — check `CHAT_RAG_MIGRATE_ON_START` |
| `docker.console` | the console is serving but cannot reach the application. Almost always `CHAT_RAG_API_URL`: it must be the *service name*, and it must be resolved at request time (see [architecture.md](architecture.md#what-ships)) |
